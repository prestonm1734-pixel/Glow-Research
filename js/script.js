...
function renderProducts(filter) {
  // The homepage leads with G3-R specifically, which is not the same
  ...
  renderProductGrid(grid, filter, { observeReveal, limit: 8, featureFirst: 'G3-R' });
}
...
/* This page's hero clip had its starter here, gated on prefers-reduced-motion
   because CSS cannot stop a video that has already begun. The homepage hero is
   a still again, so there is nothing to start and nothing to gate. The clip
   and the starter both still exist on welcome.html, in js/welcome.js. */
