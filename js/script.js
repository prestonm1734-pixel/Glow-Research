...
/* ---------- hero showpiece: studio lighting on the product ---------- */
const stageEl  = document.getElementById('orbitStage');
const vialMain = document.querySelector('.vial-main');
const bloomEl  = document.getElementById('bloom');

if (stageEl && vialMain) {
  let tmx = 0, tmy = 0, mmx = 0, mmy = 0; // target + smoothed cursor (-0.5..0.5)

  function lightLoop() {
    mmx += (tmx - mmx) * 0.06;
    mmy += (tmy - mmy) * 0.06;

    // the product drifts a little; rotation stays small so the mirrored
    // reflection underneath keeps reading as a real reflection
    vialMain.style.transform =
      `translate(calc(-50% + ${(mmx * 26).toFixed(2)}px), calc(-50% + ${(mmy * 14).toFixed(2)}px))` +
      ` rotateY(${(mmx * 7).toFixed(2)}deg)`;

    // the backlight drifts opposite, so the light feels like it sits in
    // space behind the product rather than being stuck to it
    if (bloomEl) {
      bloomEl.style.transform =
        `translate(calc(-50% + ${(mmx * -40).toFixed(1)}px), calc(-50% + ${(mmy * -26).toFixed(1)}px))`;
    }
