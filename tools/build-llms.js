  ...
  const catalog = Object.entries(byCat).map(([cat, items]) => {
    const lines = items.map(p => {
      ...
      return `- **${p.name}**${url}: ${p.blurb} Sizes: ${p.sizes.map(s => s.mg).join(', ')}. ` +
             `From ${fmtPrice(salePrice(p.sizes[0].price))} per vial.`;
    });
...
- Testing: every lot is analysed by an independent third-party laboratory. ${ANALYSIS_LONG}.
- Certificates: ${COA_COPY.short}. ${COA_COPY.panelNote}
